...
namespace ServerSharing.Tests
{
    ...
    public class Test_013_CountTests
    {
        ...
        [Test]
        public async Task Count_001_AllCount_CountAfterAdd_ShouldCorrectCount()
        {
            var response = await CloudFunction.Post(Request.Create("COUNT", "user1", JsonConvert.SerializeObject(EntryType.All)));
            ...
            response = await CloudFunction.Post(Request.Create("COUNT", "user2", JsonConvert.SerializeObject(EntryType.All)));
            Assert.That(response.IsSuccess, Is.True);
            ...
            response = await CloudFunction.Post(Request.Create("COUNT", "user3", JsonConvert.SerializeObject(EntryType.All)));
            Assert.That(response.IsSuccess, Is.True);
            Assert.That(response.Body, Is.EqualTo("2"));
        }

        [Test]
        public async Task Count_002_DownloadedCount_UploadAndDelete_ShouldCorrectCount()
        {
            ...
            var response = await CloudFunction.Post(Request.Create("COUNT", "abracadabra", JsonConvert.SerializeObject(EntryType.Downloaded)));
            Assert.That(response.IsSuccess, Is.True);
            Assert.That(response.Body, Is.EqualTo("1"));

            await CloudFunction.Delete("test_user_2", id);

            response = await CloudFunction.Post(Request.Create("COUNT", "abracadabra", JsonConvert.SerializeObject(EntryType.Downloaded)));
            Assert.That(response.IsSuccess == true, Is.True);
            Assert.That(response.Body, Is.EqualTo("0"));
        }

        [Test]
        public async Task Count_003_DownloadedCount_DifferentUsers_ShouldCorrectCount()
        {
            ...
            response = await CloudFunction.Post(Request.Create("COUNT", "some_user_2", JsonConvert.SerializeObject(EntryType.Downloaded)));
            Assert.That(response.IsSuccess, Is.True);
            Assert.That(response.Body, Is.EqualTo("2"));
        }

        [Test]
        public async Task Count_004_TwiseRequest_ShouldSameData()
        {
            static async Task CountRequest(string userId, string expectingResult)
            {
                var response = await CloudFunction.Post(Request.Create("COUNT", userId, JsonConvert.SerializeObject(EntryType.Downloaded)));
